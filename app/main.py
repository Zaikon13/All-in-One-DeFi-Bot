...
async def get_daily_pnl() -> str:
    """Fetches today's transactions and builds Daily PnL report."""
    if not WALLET_ADDRESS:
        return "❌ WALLET_ADDRESS not configured."

    await send_telegram_message("📡 Fetching today's trades from Cronos explorer...", CHAT_ID)
    ...
    for tx in data["result"]:
        ...
        token = tx.get("tokenSymbol") or "CRO"
...
async def telegram_webhook(req: Request) -> JSONResponse:
    ...
    if text == "/start":
        await send_telegram_message("👋 Bot ready! Use /daily_pnl for today's report.")
